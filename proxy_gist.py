...
def upload_to_gist(content: str, filename: str, token: str, gist_id=None, description="", public=False):
    ...
    if gist_id:
        response = requests.patch(f"{url}/{gist_id}", headers=headers, json=data)
    else:
        response = requests.post(url, headers=headers, json=data)
    if response.status_code in [200, 201]:
        print(f"Gist 上传成功: {response.json().get('html_url')}")
        return response.json().get("html_url")
    else:
        raise Exception(f"Gist 上传失败: {response.text}")


if __name__ == "__main__":
    import os

    proxy_num = os.getenv("PROXY_NUM")
    output_format = os.getenv("OUTPUT_FORMAT", "json").lower()
    only_ip = os.getenv("ONLY_IP", "false").lower() == "true"
    check_region = os.getenv("CHECK_REGION", "false").lower() == "true"
    check_type = os.getenv("CHECK_TYPE", "false").lower() == "true"
    check_https = os.getenv("CHECK_HTTPS", "false").lower() == "true"

    gist_token = os.getenv("GIST_TOKEN")
    gist_id = os.getenv("GIST_ID")
    gist_filename = os.getenv("GIST_FILENAME", f"proxy_list.{output_format}")
    gist_description = os.getenv("GIST_DESCRIPTION", "")
    gist_public = os.getenv("GIST_PUBLIC", "false").lower() == "true"

    print("=" * 50)
    print("🌟 多源代理 IP 爬虫 for 青龙")
    print("=" * 50)
    ...
    print(f"共获取到 {len(unique_proxies)} 个唯一代理")
    selected_proxies = unique_proxies if not proxy_num else unique_proxies[:int(proxy_num)]

    from datetime import datetime
    current_time = datetime.now().strftime("%Y%m%d%H%M%S")
    output_filename = f"valid_proxies_{current_time}.{output_format}"
    ...
    save_proxies(valid_proxies, output_format, output_filename, only_ip)
    print(f"已保存 {len(valid_proxies)} 个有效代理至 {output_filename}")

    # 上传 Gist
    if gist_token:
        with open(output_filename, "r", encoding="utf-8") as f:
            content = f.read()
        upload_to_gist(content, gist_filename, gist_token, gist_id, gist_description, gist_public)
